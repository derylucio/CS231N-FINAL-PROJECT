...
def buildModel(options):
	...
	ptr_net = PointerNetwork(options.max_len, options.input_dim, options.hidden_dim, options.bidirectional, options.fc_dim, options.batch_size)
...
if __name__ == "__main__":
	...
	parser.add_option("--lr", type=float, dest="lr", default=1e-3) # TO DO : FIND BEST 
	parser.add_option("--optim", type=str, dest="optim", default="Adam") # TO DO : FIND BEST 
	...
	parser.add_option("--ptrnet_input_dim", type=int, dest="input_dim", default=7500)
	parser.add_option("--ptrnet_hidden_dim", type=int, dest="hidden_dim", default=300) # TO DO : FIND BEST 
	parser.add_option("--fc_dim", type=int, dest='fc_dim', default=1024) # TO DO : FIND BEST 
	parser.add_option("--reg", type=float, dest='reg', default=1e-3) # TO DO : FIND BEST 

	options, args = parser.parse_args()

	data = getData(options.puzzle_height, options.puzzle_width, options.batch_size)
	import cPickle as pickle
	with open(r'data_file', 'wb') as savef:
		pickle.dump(data, savef) #data = pickle.load(savef) #
